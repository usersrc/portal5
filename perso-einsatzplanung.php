...
WHERE e.KoordinatorID = m.ID";
				...
				while ($row = mysql_fetch_object($select)) {
					if(!$row->deleted && isAGreaterThanB($row->Einsatzende, date("Y-m-d"))) {
						echo "<tr><td>$row->ID</td><td>$row->Vorname</td><td>$row->Nachname</td><td>".date_mysql2german($row->Einsatzbeginn)."</td><td>".date_mysql2german($row->Einsatzende)."</td><td>$row->KVorname $row->KNachname</td><td value='$row->ID' class='remove-table'>löschen</td></tr>";
					}
				}
			?>
